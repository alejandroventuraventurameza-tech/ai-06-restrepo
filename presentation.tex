...
\newcommand{\repourl}{https://github.com/USUARIO/ai-06-restrepo}
...
\end{itemize}
\textbf{Lo que produjo}
\begin{itemize}\setlength\itemsep{1pt}
  \item Un \texttt{proposition3Spec} con las diferenciales como \emph{derivadas reales} de funciones de equilibrio.
  \item Excluye $\sh=1$ y el caso (F); $\eta\to0$ como sucesión.
\end{itemize}
\column{0.47\textwidth}
\textbf{Estado y checks} \emph{(se completa con la sesión 2)}
\begin{itemize}\setlength\itemsep{1pt}
  \item \texttt{lake build}: \emph{[pendiente]}
  \item \texttt{paper\_contribution.py check --fast}: \emph{[pendiente]}
  \item cláusulas probadas / falsas / abiertas: \emph{[pendiente]}
\end{itemize}
...
\begin{itemize}\setlength\itemsep{1pt}
  ...
  \item La cláusula del umbral quedó codificada \emph{tal cual está impresa}.
\end{itemize}
\end{columns}
\end{frame}
...
\column{0.4\textwidth}
\centering
\IfFileExists{hand/threshold.jpg}{\includegraphics[width=\linewidth,height=0.62\textheight,keepaspectratio]{hand/threshold.jpg}}{%
\fbox{\parbox[c][0.5\textheight][c]{0.9\linewidth}{\centering\small Derivación a mano\\(\texttt{hand/threshold.jpg})\\Teorema 3.3 y despeje de $\Kh$}}}
\end{columns}
\end{frame}
