...
- Source version / local files inspected: NBER WP 22252, June 2017 revision,
  87 pages; pinned PDF and `pdftotext -layout` extraction recorded in
  `docs/SOURCE_INVENTORY.md`.
- Source/version mismatch notes: none; PDF metadata title and SHA-256 match the
  maintainer pin.
- Complete named-result ledger status:
  complete source-first inventory with every unselected named result assigned
  a maintainer-approved scope exclusion in the v11 preparation config.
- Formula sanity check:
  - Signs, constants, normalizations, quantifiers, domains: checked for all P3
    atoms. Corrected `B = Btilde^((sigma-1)/(sigmaHat-1))` after maintainer
    review; no dependent definition changed.
...
- [x] The proof-obligation dependency order below is acyclic and assigns one
      owning module to each result; concurrent agents do not share an item.
- [x] One focused Lean claim-graph and raw-source statement preflight has exposed the exact
...
|---:|---|---|---|---|---|
| 1 | Proposition 3 (both bullets) | Section 2.1; Assumptions 1--3; Proposition 2 quantities; (B9)--(B10) | `ProofInterface.lean` | yes, pending proof-session recheck | `sorry` permitted for this session |
...
|---|---|---|---|---|---|
| Proposition 3, printed p. 13 | `proposition3Spec` -> `proposition3` | draft-preflight digest in ignored review trace | preflight matches subject to memoed assumptions/domain changes | grouped concrete premises exposed | `sorry` |
